 ...
 * @param [in] channel the channel to do the RPC on
 * @param [in] queue queue
 * @param [in] exchange exchange// Declare a durable queue
    amqp_bytes_t queue = amqp_cstring_bytes(queue_name.c_str());
 ...
 */
    ...
    amqp_queue_bind(conn, 2, queue,amqp_cstring_bytes("amq.direct"), amqp_cstring_bytes("more info"), amqp_empty_table);
 ...
 */
  // Start consuming messages
    amqp_basic_consume(conn, 2, queue, amqp_cstring_bytes("other CDP"), 0, 0, 0, amqp_empty_table);
    reply = amqp_get_rpc_reply(conn);
    check_amqp_reply(reply);
